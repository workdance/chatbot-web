...
const MessageBase: React.FunctionComponent<MessageBaseProps> = (props) => {
  ...
  function findParentOfLastElement(node: HTMLElement | ChildNode | null): any {
    ...
    if (!node.hasChildNodes()) return node.parentNode;
  ...
  function addTyping() {
    ...
    if (lastChildren && lastChildren.id !== typingDomId) {
      ...
      if (ReactDOM.createRoot) {
        ...
        ReactDOM.render(<Typing color={lastChildren?.tagName === 'CODE' ? '#fff' : ''} />, dom);
      }
    }
  }
  ...
  function renderMsgHeader() {
    ...
    return (
      <div
        className={`${styles.message}__content__header`}
        style={flexFlowStyle}
      >
        {header}
      </div>
  ...
  function renderMsgFooter() {
    ...
    return (
      <div
        className={`${styles.message}__content__footer`}
        style={flexFlowStyle}
      >
        {footer}
      </div>
  ...
  return (
    <div
      ...
      style={Object.assign({}, flexFlowStyle, style )}
    >
      {renderAvatar()}
